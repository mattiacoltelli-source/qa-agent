...
test.describe("Spot — pannello meteo (mockato)", () => {
  test("giornata serena: headline e statistiche coerenti con i dati mockati", async ({ page }) => {
    ...
    const stats = page.locator(S.statsGridStat);
    await expect(stats).toHaveCount(4);
    await expect(page.locator(S.statsGrid)).toContainText("27°");
    await expect(page.locator(S.statsGrid)).toContainText("10 km/h");
  });
  ...
  test("vento forte: avviso a valutare ancoraggi riparati", async ({ page }) => {
    ...
    await expect(page.locator(S.weatherAlert)).toContainText("vento forte", { timeout: 10_000 });
  });
  ...
  test("API meteo tutte giù: l'app degrada a 'Meteo non disponibile', senza crash né dati inventati", async ({
    ...
    await expect(page.locator(S.statsGrid)).not.toContainText("NaN");
    await expect(page.locator(S.statsGrid)).toContainText("—");
  });
});
